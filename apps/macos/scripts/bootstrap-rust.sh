...
if [ "$INSTALLED_RUSTUP_VERSION" != "$RUSTUP_VERSION" ]; then
  INSTALL_DIR=$(mktemp -d "$MIX_TOOL_ROOT/rustup-bootstrap.XXXXXX")
...
RUSTUP_HOME="$RUSTUP_DIR" CARGO_HOME="$CARGO_DIR" "$CARGO_DIR/bin/rustup" component add rustfmt clippy --toolchain "$RUST_VERSION"
