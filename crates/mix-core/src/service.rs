...
impl MixService {
    ...
    pub fn state(&self) -> Result<Snapshot> {
        ...
        for (name, client) in &config.apps {
            ...
            for issue in &issues {
                // An empty account list is the normal first-run state. It
                // drives onboarding through `needs_setup`, but is not a
                // health failure that should turn the global status amber.
                if matches!(issue.code, ClientIssueCode::NoProfiles) {
                    continue;
                }
                health_issues.push(HealthIssue {
                    app: name.clone(),
...
mod tests {
    ...
    fn switch_uses_the_observed_account_and_route_when_saved_active_state_is_stale() {
        ...
        let live = root.path().join("codex");
        fs::create_dir_all(&live).unwrap();
        let hair_free = json!({"OPENAI_API_KEY":"hair-free-key"});
        let yusu = json!({
            "tokens":{"account_id":"yusu-account","access_token":"yusu-token"}
        });
        let momo = json!({
            "tokens":{"account_id":"momo-account","access_token":"momo-token"}
        });
        let vault = Arc::new(MemoryVault::default());
        let service =
            MixService::with_vault(root.path().join("mix/config.json"), vault.clone()).unwrap();
        ...
        fs::write(
            live.join("auth.json"),
            serde_json::to_vec(&hair_free).unwrap(),
        )
        .unwrap();
        service
            .capture_current_account("codex", None, Some("Hair Free"))
            .unwrap();

        fs::write(live.join("config.toml"), "model = \"yusu\"\n").unwrap();
        fs::write(live.join("auth.json"), serde_json::to_vec(&yusu).unwrap()).unwrap();
        service
            .capture_current_account("codex", None, Some("yusu"))
            .unwrap();

        fs::write(live.join("config.toml"), "model = \"momo\"\n").unwrap();
        fs::write(live.join("auth.json"), serde_json::to_vec(&momo).unwrap()).unwrap();
        service
            .capture_current_account("codex", None, Some("momo"))
            .unwrap();

        let yusu_reference = service.store.load().unwrap().apps["codex"].profiles["account-2"]
            .secret_files["auth.json"]
            .clone();
        let yusu_before = vault.get(&yusu_reference).unwrap();
        fs::write(
            live.join("config.toml"),
            "model_provider = \"custom\"\n[model_providers.custom]\nname = \"Hair Free\"\nbase_url = \"https://hairfree.example/gateway/v1\"\nwire_api = \"responses\"\n",
        )
        ...
        assert_eq!(vault.get(&yusu_reference).unwrap(), yusu_before);
        assert_eq!(
...
"#,
    ...
    fn codex_switch_keeps_every_credential_and_provider_route_paired() {
        let cases = [
            ...
            (
                "official-to-hair-free",
                "model = \"official\"\n",
                json!({"tokens":{"account_id":"official-a","access_token":"official-token"}}),
                "model_provider = \"custom\"\n[model_providers.custom]\nname = \"Hair Free\"\nbase_url = \"https://hairfree.example/gateway/v1\"\nwire_api = \"responses\"\nrequires_openai_auth = true\n",
                json!({"OPENAI_API_KEY":"hair-free-key"}),
                "custom",
            ),
            (
                "custom-to-official",
