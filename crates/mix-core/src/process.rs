...
mod tests {
    ...
    fn process_fixture() {
        if std::env::var_os("MIX_PROCESS_FIXTURE").is_some() {
            // Keep the fixture alive well beyond every discovery deadline. A
            // duration equal to the deadline makes the test depend on runner
            // startup speed, especially under cross-architecture CI load.
