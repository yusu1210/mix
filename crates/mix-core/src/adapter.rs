...
mod tests {
    ...
    fn diagnostic_endpoint_host_follows_the_effective_provider_route() {
        let partner_gateway = toml::from_str::<toml::Table>(
            "model_provider = \"custom\"\n[model_providers.custom]\nbase_url = \"https://Partner-Gateway.example/v1\"\n",
        )
        .unwrap();
        assert_eq!(
            codex_endpoint_host(&partner_gateway).as_deref(),
            Some("partner-gateway.example")
        );
    ...
    fn codex_config_rejects_secrets_disguised_as_environment_references() {
        for config in [
            "[model_providers.gateway]\nenv_key = \"literal credential value\"\n",
            ...
            assert_eq!(error.code, ErrorCode::MixSensitiveDataRejected);
            assert!(!error.to_string().contains("literal credential value"));
    ...
    fn codex_resume_reads_the_provider_from_an_isolated_runtime() {
        ...
        fs::write(
            ...
            "model_provider = \"partner-gateway\"\n",
        )
        .unwrap();
        let client = ClientConfig {
            adapter: AdapterKind::Codex,
        ...
        assert_eq!(argv[2], "model_provider=\"partner-gateway\"");
    }
